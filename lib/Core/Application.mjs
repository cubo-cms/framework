  ...
  process(request) {
    return new Promise((resolve, reject) => {
      ...
      _app_.router(_app_.get('route')).then(async (router) => {
        ...
        // Set cookies
        response.cookie = [ Cookie.serialize({ sessionId: cookie.sessionId, Path: '/', Secure: true }) ];
        if(cookie.accessToken || response.accessToken)
          response.cookie.push(Cookie.serialize({ accessToken: response.accessToken || cookie.accessToken, Path: '/', Secure: true }));
        // Provide response
        resolve(response);
      }).catch((error) => {
        console.debug(error);
