...
export default class Application extends Core {
  ...
  /** @function session()
    *
    * Getter function session - returns session object
    *
    * @return {object}
    **/
  get session() {
  ...
  process(application, request) {
    return new Promise((resolve, reject) => {
      ...
      // Invoke the router
      application.router({ method: request.method, path: request.url, payload: request.payload, contentType: request.headers['content-type'] })
        .then(async (router) => {
          // Load route list
          router.route = Core.load(application.get('route'));
          // Parse the request
          let result = router.parse();
          if(result.ok === false) {
            // This means something went wrong, return the data
            return result;
          } else {
          ...
          if(controller.ok === false) {
            ...
            if(controller.get('accessToken')) {
              // Access token passed, find existing session
              controller.session = Session.find(controller.get('accessToken'), application.#session);
            } else {
