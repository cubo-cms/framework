...
export default class Controller extends Core {
  ...
  method = {
    get: async function(controller) {
      ...
      let access = Controller.access[controller.get('session').userRole]
      ...
      controller.set('data', await model.invokeMethod());
      // Invoke view render
  ...
  static construct(dataType = '', data = {}, caller = null) {
    let dataSource = Core.load(_app_.get('dataSource')) || {};
    let controller;
