...
export default class Controller extends Core {
  ...
  method = {
    ...
    post: async function(controller) {
      // Limit access to documents according to user role
      if(Controller.access.canCreate.includes(controller.session.userRole)) {
        // Invoke controller method
        let model = controller.model(controller.data);
        let data = await model.invokeMethod();
        return data;
      } else {
        if(controller.session.userRole === 'guest')
          return { ok: false, data: '', type: 'warning', status: 'forbidden' };
        else
  ...
  async invokeMethod() {
    ...
    if(method) {
      if(typeof this.method[method] === 'function') {
        Log.info(`Controller invokes method \"${method}\"`);
        return await this.method[method](this);
      } else {
        Log.warning(`Controller failed to invoke method \"${method}\"`);
        return { ok: false, data: '', type: 'warning', status: 'notAllowed' };
      }
    } else {
