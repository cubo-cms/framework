...
export default class Controller extends Core {
    ...
    try {
      if(this.get('dataType')) {
        ...
        if(Model.exists(dataType)) {
          Log.success(`Controller calls model \"${dataType}\"`);
          return Model.construct(dataType, data, this);
        } else {
          Log.warning(`Controller failed to call model \"${dataType}\"`);
          return false;
        }
      } else {
  ...
  callView(data = {}) {
    try {
      if(this.get('dataType')) {
        let object = this.get('dataType');
        if(View.exists(dataType)) {
          Log.success(`Controller calls view \"${dataType}\"`);
          return View.construct(dataType, data, this);
        } else {
          Log.warning(`Controller failed to call view \"${dataType}\"`);
          return false;
        }
      } else {
  ...
  static construct(dataType = '', data = {}, caller = null) {
    try {
      ...
      Log.error(`Controller failed to construct \"${dataType}\"`);
    }
  }
