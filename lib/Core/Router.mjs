...
export default class Router extends Core {
  ...
  controller(data = {}) {
    if(this.get('dataType')) {
      ...
      if(Controller.exists(dataType)) {
        Log.success(`Router invokes controller \"${dataType}\"`);
        return Controller.construct(dataType, data, this);
      } else {
        throw new FrameworkError({ message: `Router failed to invoke controller \"${dataType}\"`, type: 'warning' });
      }
    } else {
      throw new FrameworkError({ message: `Router could not determine controller`, type: 'warning' });
    }
  }
  ...
  /** @function parse(request)
    ...
    **/
  parse(request = '/') {
    Log.info(`Router is parsing ${request}`)
