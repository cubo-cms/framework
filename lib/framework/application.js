...
module.exports = class Application extends Entity {
  ...
  constructor(data = 'application.json') {
    ...
    global._app_ = this;
    Log.success({ 'message': `Initialised application`, 'class': Entity.name });
  }
  ...
  callRouter(data) {
    try {
      ...
      router.caller = this;
      Log.success({ 'message': `Called router`, 'class': Entity.name });
      return router;
    } catch(error) {
      Log.error({ 'message': `Failed to call router`, 'class': Entity.name });
      console.log(error);
      return false;
    }
  }
  ...
  process(request) {
    ...
    if(router = this.callRouter(this.get('router'))) {
      ...
      try {
        ...
        return '<pre>'+router.invokeMethod(controller)+'</pre>';
      } catch(error) {
        Log.error({ 'message': `Failed to invoke controller`, 'class': Entity.name });
      }
    }
  }
  ...
  run(server = { 'port': 8080 }) {
    ...
    http.createServer(function(request, result) {
      Log.debug({ 'message': `Processing request \"${request.url}\"`, 'class': Entity.name });
