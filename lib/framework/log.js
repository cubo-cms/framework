...
function output(message, defaultType = 'debug', colorize = true) {
  ...
  let result =
    ...
    (colorize ? logLevels[message.type].color : '') + message.type.toProperCase() + (colorize ? logLevels['reset'].color : '') + ': ' +
    message.message +
...
module.exports = class Log extends Entity {
  ...
  constructor(data) {
    super(data);
    // Preset log level
    this.set('logLevel', this.get('logLevel'), this.#defaultLogLevel);
    // Preset output target
    this.set('logTarget', this.get('logTarget'), this.#defaultTarget);
  }
