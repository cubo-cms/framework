  ...
  invokeController() {
    try {
      if(this.get('controller')) {
        console.log('This should now start controller '+this.get('controller')+'Controller');
        const controller = eval('new Cubo.'+this.get('Controller')+'Controller');
        return controller;
      }
    } catch(error) {
      console.log({ 'type': 'error', 'module': moduleName, 'message': `Cannot find controller \"${this.get('Controller')}\"` });
    }
  }
}
