...
module.exports = class Router extends Entity {
  ...
  _default = {
    ...
    method: 'getAll',
    route: {
      "/": { "controller": "Article", "method": "getAll" },
      "/{{controller}}": { "controller": "{{controller}}", "method": "getAll" },
      "/{{controller}}:{{name}}": { "controller": "{{controller}}", "method": "get", "name": "{{name}}" }
    }
  }
  /** Class constructor
    *
  ...
  parse(request = '/') {
    ...
    for(const [route, value] of Object.entries(this.route || this._default.route )) {
