...
const methods = {
  get: async function(driver) {
    try{
      driver.pre = Core.load(driver.get('uri'));
      driver.post = [];
      ...
      if(driver.get('id'))
        return driver.post[0];
      else
        return driver.post.slice((driver.get('page') - 1) * driver.get('pageSize'), driver.get('page') * driver.get('pageSize'));
    } catch(error) {
      Log.error(error);
    }
  }
